...
public class StockObjectSetIterator implements DataSetIterator {
	...
	private List<StockObject> readStockObjectFromFile (String filename) {
        ...
        try {
            ...
            for (int j = 1; j < list.size(); j++ ) {// String[] arr : list) {
                ...
                StockObjectList.add(new StockObject(Double.parseDouble(arr[0]), nums[0], nums[1], nums[2], nums[3], nums[4], nums[5]));
            }
        } catch (IOException e) {
            e.printStackTrace();
        }
